...
const SystemConfigManager: React.FC = () => {
  ...
  const handleQuery = async (params: any) => {
    const result: Protocol.RestResult = await requestGet<Protocol.RestResult>('/aio/sc/list', params);
    return {
  ...
  const handleGet = async (record: any, callback: handleGetCallback) => {
    const result: Protocol.RestResult = await requestGet<Protocol.RestResult>('/aio/sc/get', { "id": record.id });
    if (result?.success) {
  ...
  const handleSave = async (record: any) => {
    const result: Protocol.RestResult = await requestPost<Protocol.RestResult>(addStatus ? '/aio/sc/add' : '/aio/sc/edit', record);
    if (result?.success) {
  ...
  const handleRemove = async (record: any) => {
    const result: Protocol.RestResult = await requestPost<Protocol.RestResult>('/aio/sc/remove', { "id": record.id });
    if (result?.success) {
      setFormVisible(false);
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item
                name="configKey"
                label="配置项"
                rules={[{ required: true, message: '配置项不能为空' }]}
              >
